...
def _run_self_check_body(*, quick: bool = False) -> int:
    ...
    live = auto.measure_live_state(quick=quick)
    # One blocking plan + cheap loop peek — skip second build_plan(loop=True)
    # (extra ~17ms + orchestrate_cursor +3/tick). Preview needs one format_prompt.
    ...
    try:
        ...
        pool_dir = auto.ROOT / rel
        on_disk = sum(1 for i in range(hub_pool) if (pool_dir / f"{prefix}-{i}").is_dir())
        if on_disk < hub_pool:
            tips.append(
