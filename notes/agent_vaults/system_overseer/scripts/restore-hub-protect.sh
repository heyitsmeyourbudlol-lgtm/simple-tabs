...
live_bad() {
  ...
  case "$f" in
    ...
    peer_product_forge.py)
      ...
      return 1 ;;
    peer_orchestrate.py|peer_commands.py)
...
# OVERSEER_LOCAL_JSON_CAP_2026_09_04 — Mac rsync rewrites peers/hub_worker_pool to 12; restore vault SoT.
# OVERSEER_BASE_JSON_CAP_2026_09_04 — Mac also rewrites base automation.config.json to 16/12.
_peers_inflated() {
  python3 -c "import json,sys; d=json.load(open(sys.argv[1])); sys.exit(0 if int(d.get('max_parallel_peers',0) or 0)>8 or int(d.get('hub_worker_pool',0) or 0)>8 or int(d.get('parallel_peer_floor',0) or 0)>8 else 1)" "$1" 2>/dev/null
}
