...
def _label_to_systemd_unit(label: str) -> str | None:
    # OVERSEER_FACTORY_OVERSIGHT_SYSTEMD_2026_09_04 — map oversight too (parity with
    # factory_progress); peer/improve-only left Linux oversight invisible.
    low = (label or "").lower()
            ...
            for line in (proc.stdout or "").splitlines():
                if label in line:
                    ...
                    if len(parts) >= 2 and parts[0] != "-":
                        return True
            if "pid" in (proc.stdout or "").lower():
                return True
    except (OSError, subprocess.TimeoutExpired):
        pass
...
def _log_has_recent(
    ...
    *,
    ...
    try:
        tail = path.read_text(encoding="utf-8", errors="replace")[-tail_bytes:]
    except OSError:
...
def _log_has_recent_any(
    ...
    *,
    ...
    if age > max_age_sec:
        return False, f"{path.name} stale ({age:.0f}s)"
    try:
        tail = path.read_text(encoding="utf-8", errors="replace")[-tail_bytes:].lower()
    except OSError:
        return False, f"cannot read {path.name}"
    for needle in needles:
        if needle.lower() in tail:
            return True, f"{path.name} age={age:.0f}s; matched '{needle}'"
...
def _probe_ctx(signals: Any) -> dict[str, Any]:
    ...
    from automation_improve import IMPROVE_LABEL, SCRIPTS as IMP_SCRIPTS
    ...
    return {
        ...
        "worktree_path": IMP_SCRIPTS / "peer_worktree.py",
    }
