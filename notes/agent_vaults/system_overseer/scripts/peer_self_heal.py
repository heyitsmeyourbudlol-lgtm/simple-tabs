...
def _peer_daemon_running() -> bool:
    """True when the disk-canonical peer LaunchAgent is running."""
    if sys.platform == "darwin":
        label = _live_peer_label()
        return bool(label) and _launchctl_running(label)
    return _systemd_user_active("peer-loop.service")


def _improve_daemon_running() -> bool:
    """True when the disk-canonical improve LaunchAgent is running."""
    improve_label = _live_improve_label()
    if sys.platform == "darwin":
        return bool(improve_label) and _launchctl_running(improve_label)
    return _systemd_user_active("improve-loop.service")


def _repo_research_daemon_running() -> bool:
    """True when repo-research-loop is up (Linux systemd / macOS LaunchAgent).
    ...
    """
    if sys.platform == "darwin":
        label = f"com.togi.{cfg_mod.CFG.get('config_namespace', 'automation-hub')}-repo-research-loop"
        return _launchctl_running(label)
    return _systemd_user_active("repo-research-loop.service")
...
def dual_namespace_collision() -> dict[str, list[str]]:
    ...
    return {"peer": rogue_peer, "improve": rogue_improve}
...
def _improve_label() -> str:
    try:
        ...
        return improve.IMPROVE_LABEL
    except Exception:  # noqa: BLE001
        return f"com.togi.{cfg_mod.CFG.get('config_namespace', 'automation-hub')}-improve-loop"
...
def _heal_peer_daemon(_reg: dict[str, Any]) -> str:
    if sys.platform != "darwin":
        try:
            ...
            if _peer_daemon_running():
                return f"daemon heal flock busy: {exc}; peer already active (skip restart)"
...
def _heal_improve_daemon(_reg: dict[str, Any]) -> str:
    if sys.platform != "darwin":
        try:
            ...
            if _systemd_unit_usable("improve-loop.service"):
                retry = _systemd_restart("improve-loop.service")
                return f"daemon heal flock busy: {exc}; {retry}"
            return f"daemon heal flock busy: {exc}"
    ...
    try:
        with _daemon_heal_flock():
            result = _kickstart_or_install(label, install_fn=improve._cmd_install_body)
            legacy = _bootout_legacy_peer_labels()
            rogue = _heal_rogue_oversight()
            if not _wait_launchctl_running(label, attempts=4, delay_sec=0.4):
                retry = _kickstart(label)
        ...
        plist = Path.home() / "Library" / "LaunchAgents" / f"{label}.plist"
        if plist.is_file() or improve.PLIST_PATH.is_file():
            retry = _kickstart(label)
            if _wait_launchctl_running(label, attempts=4, delay_sec=0.4):
                return f"daemon heal flock busy; kickstart ok: {retry}"
            return f"daemon heal flock busy: {exc}; kickstart {retry}"
...
def _merge_history(found: list[Bottleneck], registry: dict[str, Any]) -> list[Bottleneck]:
...
def scan_bottlenecks(*, daemons: dict[str, bool] | None = None) -> list[Bottleneck]:
    ...
    try:
        ...
        improve_label = improve.IMPROVE_LABEL
    except Exception:  # noqa: BLE001
        improve_label = f"com.togi.{cfg_mod.CFG.get('config_namespace', 'automation-hub')}-improve-loop"
    ...
    improve_label = _live_improve_label() or improve_label
    ...
    if horizon_age is not None and horizon_age > 900 and improve_running:
        found.append(
            Bottleneck(
                id="horizon_stale",
                category="improve",
    ...
    try:
        ...
        if dupes >= 3 or self_heal_lines >= 5:
            found.append(
                Bottleneck(
                    id="queue_spam",
                    category="queue",
                    ...
                    evidence=f"dupes={dupes} self_heal={self_heal_lines}",
...
def _is_last_cycle_deferred_poison(last_cycle: dict[str, Any]) -> bool:
    ...
    try:
        ...
        return bool(transcript.is_last_cycle_poison(last_cycle))
    except Exception:  # noqa: BLE001
        ft = str(last_cycle.get("failure_type") or "").strip()
        return last_cycle.get("verify_ok") is True and ft == "deferred"
...
def _heal_noop_stall(_reg: dict[str, Any]) -> str:
    removed, _actions = auto.compact_executable_queue(write=True, max_active=12)
    sig = _touch_signal()
    return f"compact {removed} line(s); {sig}"
...
def _hub_protect_restore_paused() -> bool:
    ...
    if (hub / "RESTORE_PAUSED").is_file() or (hub / "hub-protect" / "RESTORE_PAUSED").is_file():
        return True
    # OVERSEER_ROOT_RESTORE_PAUSED_SELF_HEAL_2026_09_04 — repo-root twin when
    # peer_land_hold is mid-clobber / import-failed (land_hold already covers ROOT).
    if (ROOT / "RESTORE_PAUSED").is_file():
        return True
...
def apply_heals(
    ...
    *,
    ...
    """Run mechanical heals for detected bottlenecks (respecting cooldown)."""
    ...
    for bn in bottlenecks:
        ...
        # Critical daemons: never leave peer/improve down for a full cooldown window.
        if on_cooldown and not (
            bn.id in _CRITICAL_DAEMON_HEALS and _daemon_heal_still_down(bn)
        ):
            bn.status = "deferred"
...
def run_cycle(
    *,
    ...
    bottlenecks = _merge_history(raw, registry)
    actions: list[str] = []
