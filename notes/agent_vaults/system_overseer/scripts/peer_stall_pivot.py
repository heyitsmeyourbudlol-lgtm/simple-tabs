...
def mark_continuum_kit_refresh() -> None:
    ...
    try:
        ...
        # Full precision — ``.3f`` rounded stamps into the future ~40% of the time,
        # so continuum_kit_fresh (age >= 0) rejected a just-written stamp.
        CONTINUUM_KIT_TS.write_text(f"{time.time():.6f}\n", encoding="utf-8")
    except OSError:
        pass
...
def continuum_kit_fresh(*, ttl_sec: float | None = None) -> bool:
    ...
    try:
        ...
        # Negative age: stamp slightly ahead (legacy .3f round / clock skew) — still fresh.
        return age < ttl
