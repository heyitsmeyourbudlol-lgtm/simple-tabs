...
DEFAULT_FALLBACK_POLL_SEC = transcript.FALLBACK_POLL_SEC
...
def _noop_remaining_or_clear(*, state: dict, log_fn, backoff_sec: float | None = None) -> float:
    """Noop backoff seconds left — cleared when autonomy_never_delay_on_error."""
    import peer_transcript as transcript

    sec = backoff_sec if backoff_sec is not None else effective_noop_backoff_sec()
    remaining = transcript.noop_backoff_remaining(state, backoff_sec=sec)
    if remaining <= 0:
        return 0.0
    if not bool(auto.CFG.get("autonomy_never_delay_on_error", True)):
        return remaining
    try:
        import peer_error_adapt as err_adapt

        err_adapt.clear_autonomy_delays(log_fn=log_fn, reason="noop-bypass-on-error")
        log_fn("intelligence: noop backoff cleared — errors must not delay autonomy")
    except Exception:  # noqa: BLE001
        pass
    return 0.0
...
def maybe_auto_commit_after_verify(*, log_fn, note: str = "") -> bool:
    """Commit all changes after verify passes. No-op when disabled or already clean."""
    ...
    for cwd in _commit_paths_after_verify():
        ...
        if add is None or add.returncode != 0:
            err = ((add.stderr if add else "") or (add.stdout if add else "") or "git add failed").strip().splitlines()
            log_fn(f"auto-commit: git add FAIL ({cwd}) — {err[-1] if err else (add.returncode if add else '?')}")
            return False
        ...
        if commit.returncode == 0:
            ...
            if auto.auto_push_after_commit_enabled():
                ...
                if push.returncode == 0:
                    ...
                    log_fn(f"auto-push: FAIL ({cwd.name}) — {err[-1] if err else push.returncode}")
                    return False
            ...
            log_fn(f"auto-commit: FAIL ({cwd.name}) — {err[-1] if err else commit.returncode}")
            return False
...
def _mechanical_pre_dispatch(*, log_fn, role_id: str = "orchestrator") -> bool:
    """Compact + self-heal soft stalls + plan-gate. Soft noop does not block."""
    try:
    ...
    try:
        # Clear noop flag so diagnose + gate stay soft while agents advance queue.
        ...
        if state_path.is_file():
            ...
            if isinstance(lc, dict) and lc.get("noop"):
                ...
                log_fn("pre-dispatch: cleared noop stall — agents may advance queue")
    except Exception as exc:  # noqa: BLE001
    ...
    if auto.parallel_agent_dispatch_enabled():
        try:
            ...
            if assignments and not plan.stop:
                # Never launch/ensure more hub niches than worker_pool_size (floor may be 48).
...
def _prepare_continuous_prompts(*, quick: bool, log_fn) -> None:
    ...
    try:
        ...
        signals = improve.gather_signals(quick=quick, research=True, refresh_trends=False)
...
def _should_skip_emit_ensure(target: int) -> bool:
    """Skip ensure when floor ready and last ensure is within EMIT_ENSURE_TTL_SEC."""
    if _emit_ensure_at <= 0.0:
        return False
    if (time.monotonic() - _emit_ensure_at) >= EMIT_ENSURE_TTL_SEC:
        return False
    return _hub_parallel_floor_ready(target)
    ...
    try:
        ...
        if _should_skip_emit_ensure(target):
            ...
            log_fn(
                ...
                f"{EMIT_ENSURE_TTL_SEC:.0f}s, age={age:.1f}s) — list only"
            )
        else:
            pool = peer_worktree.ensure_parallel_pool(count=target, log_fn=log_fn)
            _emit_ensure_at = time.monotonic()
            if pool:
                log_fn(f"worktree pool: {len(pool)} hub trees ready (target {target} peers)")
    except Exception as exc:  # noqa: BLE001
        log_fn(f"worktree pool: ensure skipped ({exc})")
    try:
        entries = peer_worktree.list_worktrees(ROOT)
    except RuntimeError as exc:
        log_fn(f"worktree: inventory failed ({exc})")
        return
    extras = peer_worktree.parallel_entries(entries, root=ROOT)
    log_fn(
        f"worktree: {len(entries)} registered, {len(extras)} parallel "
        "(peer_worktree.list_worktrees)"
    )
    for e in extras[:6]:
        label = e.branch or ("detached" if e.detached else e.head[:12] or "?")
        log_fn(f"worktree: parallel {e.path} ({label})")
    if not extras:
        log_fn(
...
def _run_local_continuous(*, quick: bool, log_fn, state: dict) -> bool:
...
def run_forever(
    *,
    ...
    fallback_poll_sec: float = DEFAULT_FALLBACK_POLL_SEC,
    ...
    if watcher.setup(repo_root=ROOT):
        log_fn(
            f"event-driven: kqueue on transcript dir + {transcript.SIGNAL_PATH.name} "
            f"(touch to wake)"
        )
    else:
    ...
    try:
        while True:
            ...
            open_n = len(auto.loop_work_items(context_md, queue_md, live=live).open_items)
            ...
            if mode == "background" and has_work and noop_left <= 0:
                ...
                if can_agent:
                    ...
                    if auth_failed:
                        ...
                        if _run_local_continuous(quick=quick, log_fn=log_fn, state=state):
                            continue
                    else:
                        lc = state.get("last_cycle") or {}
                        if lc.get("verify_ok") and not lc.get("noop"):
                            continue
