...
def _lean_automation_verify_commands(commands: list[str] | None, *, stack: str | None = None) -> list[str]:
    ...
    # OVERSEER_LEAN_NONET_2026_09_04 — 9 required; beat must restore this needle.
...
def generate_local_profile(root: Path, signals: ProjectSignals) -> dict[str, Any]:
    ...
    profile: dict[str, Any] = {
        ...
        "verify_commands": _lean_automation_verify_commands(
...
def apply_local_profile(root: Path, profile: dict[str, Any], *, write: bool = False) -> dict[str, Any]:
    ...
    # Belt-and-suspenders: never persist full discover on automation hub local.json
    # (concurrent daemons / stale imports used to recontaminate after lean writes).
    if (root / "profiles" / "automation.json").is_file() or auto.CFG.get("task_profile") == "automation":
...
def heal_verify_commands(root: Path, signals: ProjectSignals, *, quick: bool = False, write: bool = False) -> list[str]:
    ...
    cached = existing_verify if (quick and existing_verify) else None
...
def _sync_drift_lists(context_md: str, work_md: str) -> list[str]:
    """Drift = Remaining vs Active/phased opens (Backlog stays out of Remaining)."""
    ctx_items = auto.remaining_work_items(context_md)
...
def heal_queue_drift(*, root: Path, write: bool = False) -> tuple[list[str], list[str]]:
    ...
    ctx_items = auto.remaining_work_items(new_context)
    ...
    warnings = _sync_drift_lists(new_context, new_work)
...
def export_tarball(out_dir: Path | None = None) -> Path:
    ...
    with tarfile.open(out_path, "w:gz") as tar:
        for rel in KIT_COPY_PATHS:
            src = hub / rel
            if src.exists():
                tar.add(src, arcname=f"automation-kit/{rel}")
    return out_path
