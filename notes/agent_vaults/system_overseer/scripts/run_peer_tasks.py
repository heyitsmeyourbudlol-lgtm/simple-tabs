...
def _prepare_verify_lane(*, log_fn=print) -> None:
    """Trim unittest + self-check storms so hub verify is not SIGKILL'd mid-run."""
...
def _wait_verify_quiet(*, log_fn=print, timeout_sec: float | None = None) -> bool:
    ...
    while time.time() < deadline:
        ...
        if agents <= agent_cap and tests <= max_tests and self_checks <= max_sc:
            if agents or tests or self_checks:
                log_fn(
                    f"verify: quiet (agents={agents}, unittest={tests}, self-check={self_checks})"
                )
            return True
        time.sleep(3.0)
    agents = len(ppd.find_agent_procs())
...
def _run_cmd_group(cmd: str, *, timeout: float) -> subprocess.CompletedProcess[str]:
    ...
    trim_unittest_storm cannot SIGKILL hub verify mid-gate (rc=-9 theater).
    """
        ...
        for cmd in load_verify_commands():
            ...
            if proc.returncode != 0:
                ...
                if "self-check" in cmd.lower() and (
                    ...
                    return -1, "deferred"
                # OVERSEER_VERIFY_SIGKILL_DEFER_2026_09_04 — storm trim / OOM
                # SIGKILL (rc<0, esp -9) is not a red test; matches adapt audit
                # OVERSEER_AUDIT_SIGKILL_SKIP. Hard-FAIL poisoned verify_ok=false.
