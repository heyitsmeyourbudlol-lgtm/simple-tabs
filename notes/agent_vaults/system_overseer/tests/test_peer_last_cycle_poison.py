...
class LastCyclePoisonTests(unittest.TestCase):
    ...
    def test_deferred_swarm_note_does_not_hold_when_lean_green(self) -> None:
        """OVERSEER_LEAN_VERIFY_RESTAMP_2026_09_04 — file-scoped helper coverage."""
        lc = {
            "verify_ok": False,
            "failure_type": "deferred",
        ...
        # Note-only swarm (no failure_type) must also clear under lean green.
        note_only = {
            "verify_ok": False,
            "note": "verify deferred (swarm/lock)",
            "ts": 99.0,
        }
        self.assertFalse(poison.holds_primary_dispatch(note_only, lean_green=True))
        self.assertFalse(poison.holds_primary_dispatch(note_only, lean_green=False))
        ...
        state = {"last_cycle": dict(lc)}
        self.assertIsNotNone(poison.restamp_after_lean_green(state, lean_green=True))
        self.assertIn("lean-verify restamp", str(state["last_cycle"].get("note") or "").lower())
