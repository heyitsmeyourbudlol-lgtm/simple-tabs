...
class IntelligenceCracksTests(unittest.TestCase):
    ...
    def test_empty_active_fingerprint_blank(self) -> None:
        """OVERSEER_EMPTY_ACTIVE_FP_BLANK — empty Active must not arm noop."""
        import peer_transcript as pt

        self.assertIn("OVERSEER_EMPTY_ACTIVE_FP_BLANK_2026_09_04", Path(pt.__file__).read_text())
        self.assertIn("OVERSEER_ACTIVE_ONLY_NOOP_FP_2026_09_04", Path(pt.__file__).read_text())
        with unittest.mock.patch.object(pt.auto, "load_work_queue_md", return_value="## Active\n\n"):
            with unittest.mock.patch.object(
                pt.auto, "close_landed_done_orphans", side_effect=lambda md: (md, 0)
            ):
                with unittest.mock.patch.object(
                    pt.auto, "promote_open_done_orphans", side_effect=lambda md: (md, 0)
                ):
                    with unittest.mock.patch.object(
                        pt.auto, "_parse_phased_work_items", return_value=[]
                    ):
                        fp, items = pt.current_queue_fingerprint()
        self.assertEqual(fp, "")
        self.assertEqual(items, [])
        # zlib empty hash is truthy — must never be used for noop when Active empty
        self.assertTrue(pt.queue_fingerprint([]))
