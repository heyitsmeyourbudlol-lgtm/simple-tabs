...
class QuietWaitRetrimTests(unittest.TestCase):
    ...
    def test_env_quiet_timeout_override(self) -> None:
        """Improve sets AUTOMATION_VERIFY_QUIET_TIMEOUT_SEC so soft-defer beats kill."""
        logs: list[str] = []
        ticks = {"n": 0}

        def _t() -> float:
            ticks["n"] += 1
            # 1st: deadline base; later: past max(15, env=12)=15s window
            return 1000.0 if ticks["n"] <= 1 else 1020.0

        with unittest.mock.patch.dict(
            os.environ,
            {"AUTOMATION_VERIFY_QUIET_TIMEOUT_SEC": "12"},
            clear=False,
        ), unittest.mock.patch.dict(
            rpt.auto.CFG,
            {"verify_quiet_timeout_sec": 90, "verify_quiet_max_unittest": 0, "verify_quiet_max_agents": 0},
            clear=False,
        ), unittest.mock.patch(
            "peer_parallel_dispatch.find_agent_procs", return_value=[object()] * 8
        ), unittest.mock.patch(
            ...
            "dgx_ram_budget.trim_self_check_storm", return_value=0
        ), unittest.mock.patch.object(rpt.time, "sleep"), unittest.mock.patch(
            "run_peer_tasks.time.time", side_effect=_t
        ):
            ok = rpt._wait_verify_quiet(log_fn=logs.append)
        self.assertFalse(ok)
        self.assertTrue(any("deferred" in m for m in logs))
        self.assertIn("OVERSEER_IMPROVE_QUIET_BEFORE_KILL_2026_09_04", Path(rpt.__file__).read_text())
...
class LeanVerifyRestampTests(unittest.TestCase):
    """OVERSEER_LEAN_VERIFY_RESTAMP_2026_09_04 — deferred swarm note vs lean green."""

    def test_deferred_swarm_note_does_not_hold_when_lean_green(self) -> None:
        ...
        self.assertFalse(poison.holds_primary_dispatch(lc, lean_green=True))
        self.assertFalse(poison.holds_primary_dispatch(lc, lean_green=False))
        restamped = poison.restamp_lean_green(lc, lean_green=True)
        self.assertIsNotNone(restamped)
        assert restamped is not None
        self.assertIs(restamped.get("verify_ok"), False)
        self.assertEqual(restamped.get("failure_type"), "deferred")
        self.assertIn("lean-verify restamp", str(restamped.get("note") or "").lower())
        self.assertFalse(poison.holds_primary_dispatch(restamped, lean_green=True))
        self.assertIsNone(poison.restamp_lean_green(lc, lean_green=False))

    def test_real_verify_fail_holds_unless_lean_green(self) -> None:
        import peer_last_cycle_poison as poison
        ...
        self.assertTrue(poison.holds_primary_dispatch(fail, lean_green=False))
        self.assertFalse(poison.holds_primary_dispatch(fail, lean_green=True))
        self.assertIsNone(poison.restamp_lean_green(fail, lean_green=True))

    def test_restamp_skips_already_green_delivery_note(self) -> None:
        """OVERSEER_LEAN_RESTAMP_SKIP_GREEN_2026_09_04 — success notes mention deferred."""
        import peer_last_cycle_poison as poison

        green = {
            "verify_ok": True,
            "noop": False,
            "note": "stall-watch lean verify green; cleared deferred swarm false-FAIL",
            "ts": 1_700_000_000.0,
            "queue_fp": "x",
            "git_head": "a",
            "rc": 0,
        }
        self.assertIsNone(poison.restamp_lean_green(green, lean_green=True))
        self.assertIsNone(poison.restamp_after_lean_green({"last_cycle": green}, lean_green=True))
        import peer_self_heal as heal

        with unittest.mock.patch.object(heal, "_lean_verify_cached_ok", return_value=True):
            self.assertFalse(heal._needs_deferred_lean_restamp(green))

    def test_restamp_after_lean_green_mutates_state_fail_closed(self) -> None:
        import peer_last_cycle_poison as poison
        ...
        msg = poison.restamp_after_lean_green(state, lean_green=True)
        self.assertIsNotNone(msg)
        lc = state["last_cycle"]
        self.assertIs(lc.get("verify_ok"), False)
        self.assertEqual(lc.get("failure_type"), "deferred")
        self.assertIn("lean-verify restamp", str(lc.get("note") or "").lower())
        self.assertFalse(poison.holds_primary_dispatch(lc, lean_green=True))
        self.assertIsNone(poison.restamp_after_lean_green(state, lean_green=False))

    def test_self_heal_wires_deferred_lean_restamp(self) -> None:
        """OVERSEER_LEAN_VERIFY_RESTAMP_SELF_HEAL_2026_09_04 — peer_self_heal path."""
        import peer_self_heal as heal

        src = (Path(__file__).resolve().parents[1] / "scripts" / "peer_self_heal.py").read_text(
            encoding="utf-8"
        )
        self.assertIn("OVERSEER_LEAN_VERIFY_RESTAMP_SELF_HEAL_2026_09_04", src)
        self.assertIn("deferred_lean_restamp", heal._HEALERS)
        self.assertTrue(callable(heal._HEALERS["deferred_lean_restamp"]))
        ...
        with unittest.mock.patch.object(heal, "_lean_verify_cached_ok", return_value=True):
            self.assertTrue(heal._needs_deferred_lean_restamp(lc))
        with unittest.mock.patch.object(heal, "_lean_verify_cached_ok", return_value=False):
            self.assertFalse(heal._needs_deferred_lean_restamp(lc))
        tagged = dict(lc)
        tagged["note"] = "verify deferred (swarm/lock); lean-verify restamp: deferred must not hold"
        with unittest.mock.patch.object(heal, "_lean_verify_cached_ok", return_value=True):
            self.assertFalse(heal._needs_deferred_lean_restamp(tagged))

        state = {"last_cycle": dict(lc), "stall_reason": "noop_backoff", "stall_since_ts": 99.0}
        saved: list[dict] = []

        def _save(s: dict) -> None:
            saved.append(dict(s))

        with unittest.mock.patch.object(heal, "_lean_verify_cached_ok", return_value=True), unittest.mock.patch(
            "peer_transcript.load_state", return_value=state
        ), unittest.mock.patch("peer_transcript.save_state", side_effect=_save), unittest.mock.patch(
            "peer_stall_pivot.clear_stall",
            side_effect=lambda s: s.pop("stall_reason", None) or s.pop("stall_since_ts", None),
        ):
            msg = heal._heal_deferred_lean_restamp({})
        self.assertIn("restamped", msg.lower())
        self.assertIn("no waiting", msg.lower())
        self.assertTrue(saved)
        out_lc = saved[-1]["last_cycle"]
        self.assertIs(out_lc.get("verify_ok"), False)
        self.assertEqual(out_lc.get("failure_type"), "deferred")
        self.assertIn("lean-verify restamp", str(out_lc.get("note") or "").lower())
        self.assertNotIn("stall_reason", saved[-1])
